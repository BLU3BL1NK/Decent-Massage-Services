...
<div class="art-content-layout">
    ...
    <div class="art-layout-cell" style="width: 100%;font-size:18px" >
        ...
        <center><table width=100% border=1 cellspacing=0>
                ...
                <?php
                    $B = 1;
                    $A1 = mysqli_query($cnn,"SELECT tblreservation.ReservationID,tblclient.FName,tblclient.LName,tblclient.Age,tblclient.Sex,tblclient.Mobile,tblservices.Type,tblreservation.Date,tblreservation.FTime,tblreservation.Type,tblreservation.`Status` FROM tblreservation INNER JOIN tblclient ON tblreservation.ClientID = tblclient.ClientID INNER JOIN tblservices ON tblreservation.ServiceID = tblservices.`No` WHERE tblreservation.Status='Confirmed' AND tblreservation.Type='Home Service' ORDER BY tblreservation.Date desc");
                    while ($A2 = mysqli_fetch_array($A1)) {
                        if($B % 2 == 1)
                            ...
                            echo "<td>" . $A2[9] . "</td>";
                            echo "<td>" . $A2[10] . "</td>";
                            echo "<td><center><a href='SendSMS.php?ReservationID=" . $A2[0] . "'><img src='images/SMS.png' style='width:25px;height:25px' title='SEND SMS'></a></td>";
                            echo "<td><center><a href='ConfirmReservation.php?ReservationID=" . $A2[0] . "'><img src='images/Confirm.png' style='width:25px;height:25px' title='CONFIRM'></a></td>";
                            echo "<td><center><a href='CancelReservation.php?ReservationID=" . $A2[0] . "'><img src='images/Cancel.png' style='width:25px;height:25px' title='CANCEL'></a></td>";
                            $B++;
                        }
                ?>
            </table></center>
